...
/**
 * Send Daily Training Email Sample WITH Audio Link
 *
 * Uses a clickable button/image approach since HTML5 audio doesn't work in all email clients
 */
...
// Demo audio - will be replaced with actual hosted training audio
    ...
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; background: #f8fafc;">
      ...
      <div style="background: white; padding: 40px; border-radius: 0 0 12px 12px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
        ...
        <div style="background: linear-gradient(135deg, #1e293b 0%, #334155 100%); padding: 30px; border-radius: 12px; margin: 20px 0; text-align: center;">
          ...
          <p style="margin: 0 0 20px 0; font-size: 20px; font-weight: bold; color: white;">Listen to Today's Training</p>
          <p style="margin: 0 0 25px 0; font-size: 14px; color: #94a3b8;">Duration: 2:04 minutes</p>

          <!-- Play Button (clickable link) -->
          <a href="${AUDIO_URL}" target="_blank" style="display: inline-block; background: #0ea5e9; color: white; padding: 18px 40px; text-decoration: none; border-radius: 50px; font-weight: bold; font-size: 18px; box-shadow: 0 4px 12px rgba(14, 165, 233, 0.4);">
            ▶️ PLAY AUDIO TRAINING
          </a>

          <p style="margin: 20px 0 0 0; font-size: 12px; color: #64748b;">
            (Opens in new tab - works on all devices)
          </p>
        </div>

        <div style="background: #fffbeb; border-left: 4px solid #f59e0b; padding: 15px; margin: 20px 0;">
          <p style="margin: 0; font-weight: bold; color: #92400e;">📝 Prefer reading? Full transcript below</p>
        </div>
        ...
        <div style="text-align: center; margin: 30px 0;">
          <a href="${AUDIO_URL}" target="_blank" style="display: inline-block; margin: 10px; padding: 12px 24px; background: #f8fafc; color: #1e40af; text-decoration: none; border-radius: 8px; border: 2px solid #e2e8f0; font-weight: bold;">
            🎧 Listen Again
          </a>
          <a href="https://reachtheapex.net/dashboard" style="display: inline-block; margin: 10px; padding: 12px 24px; background: #f8fafc; color: #1e40af; text-decoration: none; border-radius: 8px; border: 2px solid #e2e8f0; font-weight: bold;">
            📊 Go to Dashboard
          </a>
        </div>
        ...
        <!-- Note about demo audio -->
        <div style="margin-top: 20px; padding: 15px; background: #dbeafe; border-radius: 8px; border-left: 4px solid #3b82f6;">
          <p style="margin: 0; font-size: 13px; color: #1e40af;">
            <strong>📌 Demo Note:</strong> This sample uses a clickable button approach that works in ALL email clients (Gmail, Outlook, Apple Mail, mobile). Production will have professionally narrated 2-minute audio for each day.
          </p>
        </div>
      </div>
      ...
      <div style="text-align: center; margin-top: 30px; padding: 20px;">
        <p style="font-size: 12px; color: #94a3b8; margin: 0;">
          This is a sample of our Daily 2-Minute Training series with clickable audio.<br>
...
async function sendSample() {
  console.log('📧 Sending Daily Training Sample Email WITH CLICKABLE AUDIO...\n');
  ...
  try {
    ...
    const result = await resend.emails.send({
      ...
      subject: `[SAMPLE - CLICKABLE AUDIO] ${sampleEmail.subject}`,
    ...
    if (result.error) {
      ...
      console.log('📋 Check your inbox - this version has a CLICKABLE PLAY BUTTON!');
      console.log('🎧 Click the big blue button to open and play the audio.');
      console.log('💡 Works in ALL email clients (Gmail, Outlook, Apple Mail, mobile).\n');
      console.log('✅ This is the recommended approach for email audio.');
      console.log('   - Universal compatibility');
      console.log('   - Works on desktop and mobile');
      console.log('   - Opens audio in new tab/window');
      console.log('   - User has full control (pause, replay, etc.)');
    }
