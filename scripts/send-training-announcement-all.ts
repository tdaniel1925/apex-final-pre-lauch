// Send Training Announcement to ALL Reps (Email + SMS)
...
async function sendToAllReps() {
  console.log('📧 Sending training announcement to ALL reps...\n');
  ...
  console.log(`📊 Found ${distributors.length} total reps\n`);
  ...
  for (const distributor of distributors) {
    ...
    try {
      ...
      if (emailResult.error) {
        console.error(`❌ Failed to send to ${distributor.email}:`, emailResult.error);
        emailsFailed++;
      } else {
        emailsSent++;
        if (emailsSent % 10 === 0) {
  ...
  console.log(`   📊 Total sent: ${emailsSent}`);
  console.log(`   ❌ Failed: ${emailsFailed}`);
  console.log(`   📱 Reps with phone numbers: ${smsCount}\n`);

  console.log('📱 SMS MESSAGE (to be sent via Twilio/SMS service):');
  console.log('---------------------------------------------------');
  console.log(`Recipients: ${smsCount} reps with phone numbers`);
  console.log('Message:');
  console.log(`
...
- Apex Team
  ...
  console.log('✅ Training announcement sent to all reps!');
}
