// Send Pulse Products Announcement to All Distributors
// Excludes test accounts
// Run with: node send-to-all-distributors.js
...
async function getAllDistributorEmails() {
  ...
  const { data: distributors, error } = await supabase
    ...
    .neq('email', 'tdaniel@botmakers.ai')
    .neq('email', 'test@example.com')
    .neq('email', 'demo@example.com')
    .not('email', 'like', '%+test%')
    .not('email', 'like', '%test@%')
...
<head>
    ...
    <title>We're Giving You a $500/Month CRM for FREE!</title>
</head>
<body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; background-color: #f5f5f5;">
  ...
  console.log(`📧 Sending to ${recipients.length} distributors...\n`);
...
async function main() {
  ...
  console.log('⚠️  READY TO SEND TO ALL DISTRIBUTORS');
  console.log('   Press Ctrl+C within 5 seconds to cancel...\n');

  await new Promise(resolve => setTimeout(resolve, 5000));

  await sendBulkEmail(distributors);
}
