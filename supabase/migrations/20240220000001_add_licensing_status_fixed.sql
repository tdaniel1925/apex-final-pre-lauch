-- =============================================
-- Add Licensing Status System to Distributors (SAFE VERSION)
-- Can be run multiple times without errors
-- =============================================

-- Add licensing status columns to distributors table (only if they don't exist)
DO $$
BEGIN
  -- Add licensing_status column if it doesn't exist
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                 WHERE table_name = 'distributors' AND column_name = 'licensing_status') THEN
    ALTER TABLE distributors
    ADD COLUMN licensing_status TEXT NOT NULL DEFAULT 'non_licensed'
      CHECK (licensing_status IN ('licensed', 'non_licensed'));
  END IF;

  -- Add licensing_status_set_at column if it doesn't exist
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                 WHERE table_name = 'distributors' AND column_name = 'licensing_status_set_at') THEN
    ALTER TABLE distributors
    ADD COLUMN licensing_status_set_at TIMESTAMPTZ;
  END IF;

  -- Add licensing_verified column if it doesn't exist
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                 WHERE table_name = 'distributors' AND column_name = 'licensing_verified') THEN
    ALTER TABLE distributors
    ADD COLUMN licensing_verified BOOLEAN DEFAULT false;
  END IF;

  -- Add licensing_verified_at column if it doesn't exist
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                 WHERE table_name = 'distributors' AND column_name = 'licensing_verified_at') THEN
    ALTER TABLE distributors
    ADD COLUMN licensing_verified_at TIMESTAMPTZ;
  END IF;

  -- Add licensing_verified_by column if it doesn't exist
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                 WHERE table_name = 'distributors' AND column_name = 'licensing_verified_by') THEN
    ALTER TABLE distributors
...
CREATE INDEX IF NOT EXISTS idx_distributors_licensing_status ON distributors(licensing_status);
CREATE INDEX IF NOT EXISTS idx_distributors_licensing_verified ON distributors(licensing_verified) WHERE licensing_status = 'licensed';

-- Create feature access rules table if it doesn't exist
...
-- Insert initial feature access rules (use ON CONFLICT to avoid duplicates)
...
-- Enable RLS on feature_access_rules if not already enabled
ALTER TABLE feature_access_rules ENABLE ROW LEVEL SECURITY;

-- Drop existing policies if they exist (to recreate them)
DROP POLICY IF EXISTS "Anyone can view feature access rules" ON feature_access_rules;
DROP POLICY IF EXISTS "Only admins can modify feature access rules" ON feature_access_rules;
...
DO $$
BEGIN
  RAISE NOTICE 'Migration completed successfully! Licensing status system is ready.';
END $$;
