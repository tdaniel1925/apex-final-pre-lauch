...
import { RACE_TO_100_TOOLS, type RaceCategory, type RaceTopic } from '@/lib/race-to-100/tools-reference';

interface RaceToolsAccordionProps {
  onTopicClick: (topic: RaceTopic) => void;
}

export default function RaceToolsAccordion({ onTopicClick }: RaceToolsAccordionProps) {
  ...
  return (
    <div className="w-64 border-l border-slate-200 bg-white flex flex-col overflow-hidden">
      ...
      <div className="px-4 py-3 border-b border-slate-200 flex-shrink-0">
        <h3 className="text-sm font-semibold text-slate-900">Help Topics</h3>
        <p className="text-xs text-slate-600 mt-0.5">Click to ask your coach</p>
      </div>

      {/* Scrollable Categories */}
      <div className="flex-1 overflow-y-auto">
        {RACE_TO_100_TOOLS.map((category) => {
      ...
      <div className="px-4 py-3 border-t border-slate-200 bg-slate-50 flex-shrink-0">
        <p className="text-xs text-slate-600 text-center">
          {RACE_TO_100_TOOLS.reduce((sum, cat) => sum + cat.topics.length, 0)} topics available
        </p>
      </div>
    </div>
  );
