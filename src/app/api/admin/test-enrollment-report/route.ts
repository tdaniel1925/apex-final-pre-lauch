...
// Sends a sample enrollment report to test email
// =============================================
  ...
  try {
    ...
    const emailContent = `
      <div style="padding: 32px 0;">
        ...
        <!-- Footer Note -->
        <div style="margin-top: 32px; padding: 16px; background: #fef3c7; border: 1px solid #fbbf24; border-radius: 8px;">
          <p style="margin: 0; font-size: 14px; color: #92400e;">
            <strong>⚠️ TEST EMAIL:</strong> This is a test of the daily enrollment report system.
            In production, this will be sent to all executives daily at 6:00 AM CST.
          </p>
        </div>
      </div>
    `;
    ...
    const result = await sendEmail({
      ...
      subject: `[TEST] Daily Enrollment Report - ${now.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}`,
      html: finalHtml,
    });
    ...
    return NextResponse.json({
      success: true,
      message: 'Test enrollment report sent to tdaniel@botmakers.ai',
      data: {
        enrollments: {
          last24h: count24h || 0,
    ...
    console.error('Error sending test enrollment report:', error);
    return NextResponse.json(
      { error: 'Failed to send test report', details: error },
      { status: 500 }
    );
  }
}
